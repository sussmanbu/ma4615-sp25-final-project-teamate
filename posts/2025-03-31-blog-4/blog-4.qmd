...
Note: Experiment data is from 2019-2021, while census data is from 2019-2024.
The following code aggregate application level data to the state level and merge it with the census data to calculate the proportion of applications by state. The analysis aims to provide insights into the distribution of applications across states and how it relates to the population size.
...
```
# plot proportion of black-name application by state (facet by year)
Note: no difference in the proportion of black-name applications across states by the experimental design.

```{r}
merged_data %>%
  ggplot(aes(x = reorder(state, proportion_black_name), y = proportion_black_name)) +
  geom_bar(stat = "identity", fill = "skyblue", color = "black") +
  coord_flip() +
  facet_wrap(~year, scales = "free") +
  labs(
    title = "Proportion of Black-Name /Total Applications by State",
    y = "Proportion of Black-Name Applicants",
    x = "State"
  )
```
...
```{r}
merged_data %>%
  ggplot(aes(x = reorder(state, callback_rate), y = callback_rate)) +
  geom_bar(stat = "identity", fill = "skyblue", color = "black") +
  coord_flip() +
  facet_wrap(~year, scales = "free") +
  labs(
    title = "Callback Rate by State",
    y = "Callback Rate",
    x = "State"
  )
```
...
```{r}
mod_1 <-glm(callback_rate ~ age, data = merged_data, family = binomial(link = "logit"))
